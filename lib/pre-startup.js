...
exports.setup = function setup(cb) {
    ...
    mkdirp(target, function(err, created) {
      ...
      async.forEach(Object.keys(themes), function clone(name, fn) {
        ...
        fs.exists(themePath, function check(exists) {
          if (exists) return fn();
          var ref = require('url').parse(src);

          //
          // Check if the provided source is a git url.
          //
          if (ref.protocol === 'git:' || ~ref.href.indexOf('git@github')) {
            var child = spawn('git', ['clone', themes[name], themePath]);
            return child.on('exit', fn);
          }

          //
          // Theme was none of the above, copy it from node_modules.
          //
          require('ncp').ncp(
  ...
  //
  function processFile(err, results, prev) {
    if (err) return cb(err);
